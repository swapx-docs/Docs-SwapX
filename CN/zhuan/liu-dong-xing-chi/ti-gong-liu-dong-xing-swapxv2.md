...
流动性是驱动 SwapX 交易的核心机制。通过提供流动性，您将代币对质押到流动性池中，从而促成兑换（Swap），并为该代币对赚取一部分交易费用。

### 💧 流动性费用结构

SwapX-V2流动性池协议，代币兑换的默认交易费用为 **0.3%**。此费用全部按照流动性提供者（LPs）在流动性储备中的贡献比例进行分配。

***

### ➕ 如何提供流动性

要提供流动性，您需要质押一定数量的代币对。两个代币的 **最低（以美元计）价值** 将决定您可以提供的最大流动性。

在本示例中，我们将使用 **XOC** 和 **USDT**来演示该过程。
